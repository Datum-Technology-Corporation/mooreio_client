...
class TestCliRegr:
    ...
    def test_cli_regr_dsim_dry_no_target_no_ts(self, capsys):
        self.reset_workspace()
        test_project_path = Path(os.path.join(os.path.dirname(__file__), "data", "project", "valid_local_simplest"))
        results = self.regr_ip(capsys, 'dsim', test_project_path, "def_ss_tb", "sanity", True)
        self.check_regr_results(results, True, 1)

    @pytest.mark.single_process
    def test_cli_regr_dsim_dry_target_no_ts(self, capsys):
        self.reset_workspace()
        test_project_path = Path(os.path.join(os.path.dirname(__file__), "data", "project", "valid_local_targets"))
        results = self.regr_ip(capsys, 'dsim', test_project_path, "def_ss_tb", "nightly", True, 'abc')
        self.check_regr_results(results, True, 2)

    @pytest.mark.single_process
    def test_cli_regr_dsim_dry_target_ts(self, capsys):
        self.reset_workspace()
        test_project_path = Path(os.path.join(os.path.dirname(__file__), "data", "project", "valid_local_targets"))
        results = self.regr_ip(capsys, 'dsim', test_project_path, "def_ss_tb", "weekly", True, 'abc', 'special')
        self.check_regr_results(results, True, 4)

    @pytest.mark.single_process
    def test_cli_regr_dsim_wet_no_target_no_ts(self, capsys):
        self.reset_workspace()
        test_project_path = Path(os.path.join(os.path.dirname(__file__), "data", "project", "valid_local_simplest"))
        results = self.regr_ip(capsys, 'dsim', test_project_path, "def_ss_tb", "bugs", False)
        self.check_regr_results(results, False, 1)

    @pytest.mark.single_process
    def test_cli_regr_dsim_wet_target_no_ts(self, capsys):
        self.reset_workspace()
        test_project_path = Path(os.path.join(os.path.dirname(__file__), "data", "project", "valid_local_targets"))
        results = self.regr_ip(capsys, 'dsim', test_project_path, "def_ss_tb", "weekly", False, 'abc')
        self.check_regr_results(results, False, 3)

    @pytest.mark.single_process
    def test_cli_regr_dsim_wet_target_ts(self, capsys):
        self.reset_workspace()
        test_project_path = Path(os.path.join(os.path.dirname(__file__), "data", "project", "valid_local_targets"))
        results = self.regr_ip(capsys, 'dsim', test_project_path, "def_ss_tb", "nightly", False, 'abc', 'special')
        self.check_regr_results(results, False, 3)
